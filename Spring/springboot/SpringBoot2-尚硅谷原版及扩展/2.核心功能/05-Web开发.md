<img src="C:\Users\cjy\AppData\Roaming\Typora\typora-user-images\image-20210506170320622.png" alt="image-20210506170320622" style="zoom: 67%;" />
...
- - error/404.html  error/5xx.html；有精确的错误状态码页面就匹配精确，没有就找 4xx.html；如果都没有就触发白页。
  - 状态码可通过th:text="${status}"。
  - 错误信息可通过th:text="${message}"显示

- @ControllerAdvice  +  @ExceptionHandler处理全局异常；底层是**ExceptionHandlerExceptionResolver 支持的**

  ![image-20210508201549965](C:\Users\cjy\AppData\Roaming\Typora\typora-user-images\image-20210508201549965.png)
...
@ServletComponentScan(basePackages = **"com.atguigu.admin"**) :指定原生Servlet组件都放在哪里，放在主配置类中。

以下放在对应想要使用的类中，继承/实现对应的类。

@WebServlet(urlPatterns = **"/my"**)：效果：直接响应，**不经过Spring的拦截器？**<!--urlPatterns放置请求-->

@WebFilter(urlPatterns={**"/css/\*"**,**"/images/\*"**})	<!--urlPatterns放置要拦截的页面-->
...
创建MyServlet类，但是不使用注解。可用此种方式

```java
ServletRegistrationBean`, `FilterRegistrationBean`, and `ServletListenerRegistrationBean
//(proxyBeanMethods=true)	保证依赖的组件单实例
@Configuration
public class MyRegistConfig {
    ...
    public ServletRegistrationBean myServlet(){
        ...
        MyServlet myServlet = new MyServlet();	//创建一个MyServlet类

        return new ServletRegistrationBean(myServlet,"/my","/my02");	//将类和需要处理的请求传过去
    }
...
- - 原理
  - 1、WebMvcAutoConfiguration  默认的SpringMVC的自动配置功能类。存在静态资源、欢迎页.....配置
